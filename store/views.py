...
def trash_files(request):
    if(request.user.is_authenticated):
        u=User.objects.get(id=request.user.id)
        doc=list(document.objects.filter(user=u, delete=1, permanentdel=0).values())
        print(doc)
        return JsonResponse(doc, status=200, safe=False)
    else:
        msg={'msg':'Unauthorized'}
...
def all_file(request):
    if(request.user.is_authenticated and request.user.is_superuser):
        doc=list(document.objects.filter(delete=0, permanentdel=0).order_by('recent').values())
        return JsonResponse(doc, status=200, safe=False)
...
def restore(request):
    if(request.method=="POST"):
        if(request.user.is_authenticated):
            body=json.loads(request.body)
            x=document.objects.get(id=body['id'])
            x.delete=0
            x.save()
            msg={'msg':'Success'}
            return JsonResponse(msg, status=200, safe=False)
